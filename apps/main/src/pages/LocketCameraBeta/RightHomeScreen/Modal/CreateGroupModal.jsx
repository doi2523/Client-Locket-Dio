...
import { SonnerPromise } from "@/components/ui/SonnerToast";
...
const CreateGroupModal = ({ open, onClose, onCreated }) => {
  ...
  const handleCreate = async () => {
    ...
    setLoading(true);
    const promise = createGroup({ userIds: uids, initialMessage: "Hello!" });
    ...
    ${isSelected ? "ring-3 ring-yellow-400" : "ring-0"}`}
        ...
        <div className="fixed bottom-0 left-0 w-full z-[80] p-4">
          <button
            ...
            disabled={selectedUids.size === 0 || loading}
            onClick={handleCreate}
          >
            {loading ? (
              <span className="loading loading-spinner loading-sm" />
            ) : (
              `Tạo nhóm (${selectedUids.size})`
            )}
          </button>
        </div>
      </div>
